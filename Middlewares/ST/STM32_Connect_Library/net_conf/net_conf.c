...
#define DataRdyIRQ	EXTI15_10_IRQn
#endif
...
const unsigned int net_tls_sizeof_suite_structure =   sizeof(mbedtls_x509_crt_profile);
...
static int8_t SPI_WIFI_Init(uint16_t mode)
{
  ...
  if (mode == ES_WIFI_INIT)
  {
    ...
    if(HAL_SPI_Init( &hspi ) != HAL_OK)
    {
     ...
     HAL_NVIC_SetPriority((IRQn_Type)DataRdyIRQ, SPI_INTERFACE_PRIO, 0x00);
     HAL_NVIC_EnableIRQ((IRQn_Type)DataRdyIRQ);
